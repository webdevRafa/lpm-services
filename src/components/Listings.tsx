    ...
    <div className="w-full px-4 md:px-0 py-20 mt-20 md:mt-40">
      ...
      <div className="w-full max-w-[1400px] mx-auto">
        {listings.map((listing, index) => (
           ...
           {listing.gallery.slice(0, 5).map((image, index) => (
            <div 
            key={index}
            className={`transition duration-300 ease-out hover:sepia-100 ${index === 0 ? 'item item-main shadow-md' : 'item shadow-md'}`}
            >
            <img src={image} alt={`${listing.name} preview ${index + 1}`} />
            </div>
           ))}
